...
export function Sale() {
    ...
    return (
        <MultiSubContainer justifyContent={'center'} alignItems={'center'} gap={'10px'}>
            {sale.map((s) => {
                return (
                    <SaleCard>
                        <MultiSubContainer margin={'20px'} gap={'20px'}>
                            <KontinumP color={'#ffffff'}>{s.name}</KontinumP>
                            <KontinumP color={'#ffffff'} opacity={0.4} fs={'14px'}>{s.time}</KontinumP>
                        </MultiSubContainer>
                        <MultiSubContainer justifyContent={'center'} alignItems={'center'} style={{ height: '100%'}}>
