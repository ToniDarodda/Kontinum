...
class DiscountService {
  async createDiscount(
    dataDiscount: CreateDiscountType,
  ): Promise<DiscountType> {
    const { data: discount }: { data: DiscountType } =
      await Fetch.post<DiscountType>("/discount", dataDiscount);
  ...
  async getDiscount(id: string): Promise<DiscountType> {
    const { data: discount }: { data: DiscountType } =
      await Fetch.get<DiscountType>(`/discount/${id}`);
  ...
  async getAllDiscount(): Promise<DiscountType[]> {
    const { data: discount }: { data: DiscountType[] } =
      await Fetch.get<DiscountType[]>("/discount");
  ...
  async patchDiscount(
    id: string,
    patchData: PatchDiscountType,
  ): Promise<number> {
    const { data: objectModified }: { data: number } =
      await Fetch.patch<number>(`/discount/${id}`, patchData);
