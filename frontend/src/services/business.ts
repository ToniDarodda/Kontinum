...
class BusinessService {
  async createBusiness(
    businessData: CreateBusinessType,
  ): Promise<BusinessType> {
    const { data: business }: { data: BusinessType } =
      await Fetch.post<BusinessType>("/business/register", businessData);
    return business;
  }

  async loginBusiness(businessData: LoginBusinessType): Promise<string> {
    const { data: token }: { data: string } = await Fetch.post<string>(
      "/business/login",
      businessData,
    );
  ...
  async getBusiness(businessId: string): Promise<BusinessType> {
    const { data: business }: { data: BusinessType } =
      await Fetch.get<BusinessType>(`/business/${businessId}`);
  ...
  async getBusinessUser(): Promise<UserType[]> {
    const { data: users }: { data: UserType[] } =
      await Fetch.get<UserType[]>(`/business/users`);
  ...
  async patchBusiness(businessData: PatchBusinessType): Promise<number> {
    const { data: objectModified }: { data: number } =
      await Fetch.patch<number>(`/business`, businessData);
